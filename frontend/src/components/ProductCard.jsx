...
export default function ProductCard({ product, compact = false }) {
  ...
  const { favorites, toggleFavorite, addToShoppingList } = useApp()
  ...
  return (
    <div className="group glass-panel hover:shadow-glow hover:-translate-y-0.5 transition-all duration-300 overflow-hidden">
      ...
      >
        ...
        <p className="text-xs text-ink/45 dark:text-white/35 mt-0.5">{product.brand} · {product.category}</p>
            ...
            {product.tags.slice(0, 2).map((t) => (
              <span
                ...
                className={`text-[10px] font-semibold px-2 py-0.5 rounded-full ${
                  t.toLowerCase().includes('alert') || t.toLowerCase().includes('sodium') || t.toLowerCase().includes('saturated')
